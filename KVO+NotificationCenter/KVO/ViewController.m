...
- (void)subscribeOnKeyboardEvents {
    // Keyboard will show
    [NSNotificationCenter.defaultCenter addObserver:self //регистрация
                                           selector:@selector(keybaordWillShow:) //обработка
...
- (void)updateBottomContraintWith:(float) constant {
    // поднятие поля ввода на какое-то количество поинтов
